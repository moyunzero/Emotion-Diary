# Requirements: 心晴MO（v1.3 已完成 · v1.4 活跃）
...
- [x] **DOC-META-01**: README.md 与当前代码一致：应用名称 **心晴MO** 明确、功能列表与实现对齐、项目结构与 `app/`、`components/`、`features/` 等目录实际结构一致、技术栈版本与 `package.json` 一致。
- [x] **DOC-META-02**: README.en.md 与 README.md 结构对应、品牌与功能描述一致；CONTRIBUTING、SECURITY 等引用的脚本与路径正确。
- [x] **DOC-META-03**: App 上架元数据（app-description-zh、app-description-en、screenshot-guide）以 **心晴MO** 为唯一对外应用名表述一致；提审资料引用的流程与页面与代码一致。
- [x] **DOC-META-04**: preflight-checklist、review-response 等提审文档与当前命名、2.3.8/2.1a 回复模板、登录说明一致；提审资料清单可交叉核对。

## v1.4 Requirements（极简差异化体验）

**Defined:** 2026-04-07

### Simple & Distinctive（SMP）

- [x] **SMP-01**: 用户可以在“隐私轻面板”中一眼看清本地存储、云同步、AI 摘要的数据边界，并独立开关相关能力。
- [x] **SMP-02**: 用户可以通过 3 步极速流程在 30 秒内完成一条情绪记录，并在中断后继续未完成记录。
- [ ] **SMP-04**: 用户可以接收可关闭、低频、非焦虑措辞的本地提醒，且提醒不会破坏离线优先体验。（Abandoned）
- [ ] **SMP-06**: 用户可以导出"叙事卡片 2.0"，其表达保留天气/花园隐喻，避免通用统计面板样式。（Abandoned）

## v1.5 Requirements（代码质量优化）

**Defined:** 2026-04-19

### 未使用导出清理 (CLEAN-01)

- [ ] **CLEAN-01a**: 移除 AudioRecorder 未使用的 default 导出
- [ ] **CLEAN-01b**: 移除 Profile 组件未使用的类型导出

### 重复导出合并 (CLEAN-02)

- [ ] **CLEAN-02a**: 解决 AudioRecorder 组件的 default + named 同名导出问题

### Lint 警告修复 (CLEAN-03)

- [ ] **CLEAN-03a**: 修复 AudioRecorder 中未使用变量
- [ ] **CLEAN-03b**: 修复 React hooks 依赖缺失
- [ ] **CLEAN-03c**: 修复 audioSync.ts 中未使用变量

### 测试修复 (CLEAN-04)

- [ ] **CLEAN-04a**: 修复 expo-audio mock 问题导致的测试失败
...
| Feature | Reason |
|---------|--------|
| 新增业务功能模块 | v1.1/v1.5 聚焦工程治理，不扩张产品范围 |
| 一次性全仓大爆炸重构 | 风险不可控，违背可回滚原则 |
| 重型框架/工具链迁移（Nx/Turborepo、测试框架整体迁移） | 成本高且偏离本里程碑主目标 |
| v1.4 极简差异化体验（Phase 19-20） | 已放弃，用户暂时不需要新功能 |
| 大范围架构变更 | 仅清理已识别问题，不做架构重构 |
| 社交化功能（排名/打卡圈/公开动态） | 与“温柔私密、低压力”产品定位冲突 |
...
| SMP-01 | Phase 17 | Complete |
| SMP-02 | Phase 18 | Complete |
| SMP-04 | Phase 19 | Abandoned |
| SMP-06 | Phase 20 | Abandoned |
| CLEAN-01a | Phase 1 | Pending |
| CLEAN-01b | Phase 1 | Pending |
| CLEAN-02a | Phase 1 | Pending |
| CLEAN-03a | Phase 1 | Pending |
| CLEAN-03b | Phase 1 | Pending |
| CLEAN-03c | Phase 1 | Pending |
| CLEAN-04a | Phase 2 | Pending |
...
- v1.4 requirements: 4 total — 2 Complete, 2 Abandoned
- v1.5 requirements: 7 total — mapped Phases 1-2 ✓

---
*Requirements defined: 2026-03-21 (v1.1), 2026-03-22 (v1.2), 2026-03-24 (v1.3), 2026-04-19 (v1.5)*  
*Last updated: 2026-04-19 — Milestone v1.5 代码质量优化*
