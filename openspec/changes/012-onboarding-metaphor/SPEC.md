...
| ONB-02 | ~~Profile 可重看 intro~~ **已撤销** | 产品决定：个人中心不提供「重新观看介绍」入口 |
...
2. **Skip / 开始记录：** 写入 seen，关闭 Modal，确保 Record tab 激活（D-13）。

### 期望结果

- 3 屏内理解 Weather → Record → Garden 隐喻链（D-06）。
- Skip 或完成后不再 auto-show（D-12）。
- 无可见条目时 Record 页显示 inline 首记 hint，首条保存后消失（D-15）。
...
- **架构：** `services/onboardingMetaphor.ts` + `MetaphorOnboardingHost` 挂于 `app/_layout.tsx`（Stack 兄弟节点）。
- **数据：** 设备级 global key `onboarding_metaphor_v1_seen`（D-10）；不阻塞 `initI18n()`。
- **i18n（Wave 1 已交付）：** `locales/*/onboarding.json`（slides/actions/firstEntry）；`i18n/index.ts` 注册 `onboarding` namespace。
...
| D-04 | ~~Profile 重看~~ **已撤销**（2026-07-02 产品） |
...
| D-15 | hint 至首条保存 |
| D-16 | ~~Profile 重看不显示首记 hint~~ **随 D-04 撤销** |

## 验收标准

- [ ] 新用户冷启动见 ≤3 屏 intro，Skip 后 relaunch 不再 auto-show（ONB-01, ONB-05）
- [ ] ~~Profile「重新观看介绍」~~（ONB-02 已撤销）
