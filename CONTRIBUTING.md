...
4. **进行开发**：
   - 请确保代码风格与现有代码保持一致。
   - 遵循项目的 OpenSpec 规范（详见 `.planning/` 目录）。
   - 提交前请依次运行（与 CI 一致）：
     ```bash
     yarn typecheck
     yarn lint
...
文件式路由、`(tabs)` 分组及与 `components` / `features` 的分工说明见：[.planning/codebase/ENGINEERING-SYSTEM.md](./.planning/codebase/ARCHITECTURE.md) §3–§5（路由与目录约定）。

## UI 与设计原则

全产品界面气质、隐喻、反模式与抽检约定（非「通用 AI 应用」模版感）见：[.planning/domain/ui-components.md](./.planning/domain/ui-components.md)；页面壳层与安全区另见 [.planning/codebase/ENGINEERING-QUALITY.md](./.planning/codebase/TESTING.md) §3。
...
- **变更收尾**：与 [AGENTS.md](./AGENTS.md) 中「Agent 协作（变更收尾）」及 `.planning/codebase/ENGINEERING-QUALITY.md` §1「变更收尾」约定一致——替换或合并实现后及时删除无用代码、类型与未引用导出，提交前跑通 `yarn typecheck` / `yarn lint`。
